...
class CommunicationInterview(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    transcript: Mapped[list] = mapped_column(JSONType, nullable=False, default=list)
    rubric_scores: Mapped[dict] = mapped_column(JSONType, nullable=False, default=dict)
    coach_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    evidence_quotes: Mapped[list] = mapped_column(JSONType, nullable=False, default=list)
